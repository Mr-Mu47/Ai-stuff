        ...
        if cards:
            ...
            with st.container(border=True):
                ...
                if st.button("Submit Answer", type="primary", key=f"sub_{idx}"):
                    ...
                    st.session_state.eval = eval_res# python -m streamlit run App.py
import json
import datetime
import math
import io
...
# ==========================================
# 2. SCHEMAS & AUTHENTICATION FUNCTIONS
# ==========================================
...
def generate_cards_with_openai(contents_input, num_cards=5, existing_subjects=None):
    if not existing_subjects:
        existing_subjects = ["General"]
    existing_str = ", ".join(f'"{s}"' for s in existing_subjects)
    ...
    if isinstance(contents_input, Image.Image):
        ...
        user_content = [
            {"type": "text", "text": f"Generate {num_cards} flashcards from this image."},
...
def evaluate_answer(user_ans, correct_ans, question):
    ...
    try:
        response = client.beta.chat.completions.parse(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You evaluate flashcard quiz responses."},
...
with tab1:
    ...
    if st.button("Generate Flashcards", type="primary"):
        ...
        if payload:
            with st.spinner("Generating..."):
                existing = get_all_subjects(user["id"])
                res = generate_cards_with_openai(payload, card_count, existing)
                if res["success"]:
